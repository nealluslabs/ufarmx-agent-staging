...
const createRequest = asyncHandler(async (req, res) => {
    ...
    const request = new Request({
        ...
        paymentsMade: paymentsMade || [],  // Ensure it's always an empty array if not provided
        ...otherFields ,            // Spread any additional fields into the request object (if any)
       cropIdInvolvedInRequest:otherFields.cropIdInvolvedInRequest // jan 30 2026 - DONT FORGET TO ADD CROP ID TO THE FRONTEND OF BUSINESS MODULE WHEN MAKING  REQUEST - JAN 30 -DAGOGO
    });

   //find the retailer farmer - then update available balance

    const createdRequest = await request.save();

    if (createdRequest) {

      // ONLY RETAILER FARMERS CAN MAKE REQUESTS
      // Find the retailer farmer who made the request
      const retailerfarmerWhoMadeRequest = await RetailerFarmer.findOne({
        farmerId: req.body.farmerId,
      });
    
      if (retailerfarmerWhoMadeRequest) {
        // Add the request ID to the retailer farmer's requests array
        await RetailerFarmer.updateOne(
          { _id: retailerfarmerWhoMadeRequest._id },
          { $push: { requests: createdRequest._id.toString() } }
        );
      }



     //IN CASE WE NEED TO UPDATE FARMERS AS WELL
    //  const farmerWhoMadeRequest = await Farmers.findOne({
    //    farmerId: req.body.farmerId,
    //  });
    //
    //  if (farmerWhoMadeRequest) {
    //    // Add the request ID to the  farmer's requests array
    //    await Farmer.updateOne(
    //      { _id: farmerWhoMadeRequest._id },
    //      { $push: { requests: createdRequest._id.toString() } }
    //    );
    //  }
 ...
 // console.log('THE RESPONSE TO UPDATE HAS BEEN FOUND',request)
   if(request){
    ...
    if (
      ...
      // Find retailer
      const retailerObjectId = new mongoose.Types.ObjectId(req.body.retailer_id);
      const retailerFound = await Retailer.findById(retailerObjectId); 
    
      if (retailerFound) {
        console.log("BASED ON REQUEST, RETAILER FOUND--->", retailerFound)
      
    
      // Extract numeric value from strings like "39,938", "N39,932", "₦39,932"
      const amountString = String(req.body.totalAmount || "");
      const extractedAmount = Number(
        amountString.replace(/[^0-9.]/g, "")
      );
    
      const currentBalance = Number(retailerFound.availableBalance || 0);
    
      // Subtract and prevent negative balance
      const newBalance = currentBalance - extractedAmount;
      retailerFound.availableBalance = newBalance < 0 ? 0 : newBalance;
    
      retailerFound.updatedAt = new Date();
     ...
     // Find retailer farmer
                // i use findOne because I assume theres only going to be one farmer with said farmerId
     const retailerFarmerFound = await RetailerFarmer.findOne({farmerId:req.body.farmerId && req.body.farmerId});

     if (retailerFarmerFound) {
       console.log("BASED ON EQUEST, RETAILER FARMER FOUND---->",retailerFarmerFound)
      ...
      const currentBalance = Number(retailerFarmerFound.availableBalance || 1000);
    
      // Subtract and prevent negative balance
      const newBalance = currentBalance + extractedAmount;
      retailerFarmerFound.availableBalance = newBalance < 0 ? 0 : newBalance;
    
      retailerFarmerFound.updatedAt = new Date();
    
      await retailerFarmerFound.save();
    }
    ...
    //find the retailerFarmer and add to their balance - also find them in the farmers collection and add to their balance
